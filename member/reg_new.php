...
if($step == 1)
{
    if($cfg_ml->IsLogin())
    {
        ...
        // echo "<script>alert('你已经注册成功，正在跳转到登录页面！');</script>";
         require_once(DEDEMEMBER."/templets/arcticle_login.htm");
        exit();
    }
    if($dopost=='regbase')
    {
        ...
        {
            // ShowMsg("你指定的用户名 {$userid} 已存在，请使用别的用户名！", "-1");
            echo "<script>alert('用户名重复，请重新注册!');window.location.href='reg_new.php';</script>";
            // require_once(DEDEMEMBER."/templets/arcticle_register.htm");
            exit();
        }
        // 检测手机号是否存在
        $row2 = $dsql->GetOne("SELECT mid FROM `#@__member` WHERE telephone LIKE '$telephone' ");

        if(is_array($row2))
        {
            // ShowMsg("你指定的用户名 {$userid} 已存在，请使用别的用户名！", "-1");
            echo "<script>alert('该手机号已注册，请换个号码试试!');window.location.href='reg_new.php';</script>";
            // require_once(DEDEMEMBER."/templets/arcticle_register.htm");
            exit();
        }
        ...
        $inQuery = "INSERT INTO `#@__member` (`mtype` ,`userid` ,`pwd` ,`uname` ,`sex` ,`rank` ,`money` ,`email` ,`telephone`,`scores` ,
        `matt`, `spacesta` ,`face`,`safequestion`,`safeanswer` ,`jointime` ,`joinip` ,`logintime` ,`loginip` )
       VALUES ('$mtype','$userid','$pwd','$uname','$sex','10','$dfmoney','$email','$telephone','$dfscores',
    ...
    require_once(DEDEMEMBER."/templets/article_register.htm");
} else {
    if(!$cfg_ml->IsLogin())
    {
        ...
        {
             // ShowMsg('你已经登陆系统，无需重新注册！', 'index.php');
             // echo "<script>alert('你已经注册成功，正在跳转到登录页面！');window.location.href='index.php';</script>";
             // exit;
        }
    }
