...
if($cfg_notallowstr !='' && preg_match("#".$cfg_notallowstr."#i", $keyword))
{
    // ShowMsg("你的搜索关键字中存在非法内容，被系统禁止！","-1");
     echo "<script>alert('你的搜索关键字中存在非法内容，被系统禁止！');window.history.go(-1);</script>";    exit();
}

if(($keyword=='' || strlen($keyword)<2) && empty($typeid))
{
    // ShowMsg('关键字不能小于2个字节！','-1');
    echo "<script>alert('关键字不能小于2个字节 !');window.history.go(-1);</script>";
    exit();
}
...
if(!empty($lasttime) && ($lasttime + $cfg_search_time) > time())
{
    ...
    echo "<script>alert('管理员设定搜索时间间隔为".$cfg_search_time."秒，请稍后再试！');window.history.go(-1);</script>";
    exit();
}
