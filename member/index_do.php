...
{       
    ...
    if($dopost=="login")
    {
        ...
        if(CheckUserID($userid,'',false)!='ok')
        {
            ...
            echo "<script>alert('你输入的用户名不合法！');window.location.href='index.php';</script>";
            exit();
        }
        ...
        //检查帐号
        $rs = $cfg_ml->CheckUserIndex($telephone,$pwd);  
        ...
        if($rs==0)
        {
            ...
            echo "<script>alert('账号不存在，请重新输入！');window.location.href='login.php';</script>";
            exit();
        }
        else if($rs==-1) {
            ResetVdValue();
        ...
        else
        {
            ...
            if(empty($gourl) || preg_match("#action|_do#i", $gourl))
            {
                ...
                // echo "<script>alert('恭喜您登录成功，即将跳转到网站首页！');window.location.href='../';</script>";
                echo "<script>alert('恭喜您登录成功，即将跳转到会员中心！');
                     window.location.href='http://localhost/dededln/a/hytc/zlxz'</script>";
                // require_once(DEDEMEMBER."/templets/article_center.htm");
                // require_once("../templets/default/list_center.htm");
