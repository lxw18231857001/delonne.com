...
function ShowMsg($msg, $gourl, $onlymsg=0, $limittime=0)
{
    ...
    else
    {
      ...
      function JumpUrl(){
        ...
        $rmsg .= "<div style='padding:6px;font-size:12px;border-bottom:1px solid #DADADA;background:#DBEEBD url({$GLOBALS['cfg_plus_dir']}/img/wbg.gif)';'><b>德洛尼友情提示！</b></div>\");\r\n";
 ...
 function Getimg($aid, $imgwith = 344, $imgheight = 450, $num = 0 , $style = 'li')
{
    ...
    for($i = 0;$i < $num;$i++)
    {
        ...
        $imglist .= "<div class='slide'><img src=" . trim($wordcount[2][$i]) . "></div>";
        }
    }
...
// 图片集调用（产品三级页面）
 ...
 function Getimg_product($aid, $imgwith = 344, $imgheight = 450, $num = 0 , $style = 'li')
{
    global $dsql;
    $imgurls = '';
     $row = $dsql -> getone("Select imgurls From`dede_addonimages` where aid='$aid'"); //
    ...
    for($i = 0;$i < $num;$i++)
    {
        if($style == 'li'){
            //此处为输出的样式，可根据需要修改
            // $imglist .= "<a href=#" . $i . "><img src=". trim($wordcount[2][$i]) . " width=" . $imgwith . " height=" . $imgheight . "></a>";
        $imglist .= "<li><img src=". trim($wordcount[2][$i]) . "></li>";
        }
    }
    // var_dump($imglist);exit;
     return $imglist;
...
// 案例轮播调用
 ...
 function Getimg_lunbo($aid, $num = 0 , $style = 'li')
{
    global $dsql;
    $imgurls = '';
     $row = $dsql -> getone("Select imgurls From`dede_addonimages` where aid='$aid'"); //
    ...
    for($i = 0;$i < $num;$i++)
    {
        if($style == 'li'){
            //此处为输出的样式，可根据需要修改
        $imglist .= "<a href='#' style=' background:url(".trim($wordcount[2][$i]).") center 100% no-repeat;background-size:cover;'></a>";
            // $imglist .= "<li><img src=". trim($wordcount[2][$i]) . " width=" . $imgwith . " height=" . $imgheight . "></li>";
        // $imglist .= "&nbsp;&nbsp;<img src=". trim($wordcount[2][$i]) . ">&nbsp;&nbsp;";
        }else{
            $imglist .= "<a href='#' style=' background:url(".trim($wordcount[2][$i]).") center 100% no-repeat;background-size:cover;'></a>";
        }
    }
